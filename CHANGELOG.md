...
Syncthing Menu 1.0 marks the app as stable. Nothing has changed since 0.4.0.
...
- Pause and caution marks show which folders and devices need a look, on the rows and on the menu items above them.

### Changed
- Rescan All and Pause All now live at the top of the Folders and Devices menus.
- The status line reads "All Paused" with a green dot while every device is paused; orange is reserved for problems.

## [0.3.9] - 2026-09-07

### Added
- Activity rows show a file, folder, or link icon beside the name.

### Changed
- Every file change is listed individually in the Activity log, as it happens; large batches are no longer summarized as "N changes" rows, so any file can be found by search while it is still in the log.
- The Activity log keeps the most recent 5,000 rows (previously 500).
